...
public class WordLadder {
    ...
    public int ladderLength(String start, String end, HashSet<String> dict) {
        ...
        constructGraph(vertex, adjLists, dict, start, end);
        int[] distance = new int[vertex.size()];

        unweightedShortestPath(vertex, adjLists , distance, 0);

        return distance[distance.length-1]+1;
    }

    private void constructGraph (ArrayList<String> vertex, ArrayList<LinkedList<Integer>> adjLists,
        ...
        for (int i = 0; i < vertex.size(); i++) {
            ...
            String curStr =  vertex.get(i);
            for (int j = 0; j < vertex.size(); j++) {
                if(i != j) {
                    String compStr =  vertex.get(j);
                    if(isVertexAdj(curStr, compStr)) {
    ...
    private boolean isVertexAdj (String curStr, String compStr) {
        ...
        while (cur < n) {
            if(curStr.charAt(cur) == compStr.charAt(cur))
                count++;
            cur++;
        }
    ...
    private void unweightedShortestPath (ArrayList<String> vertex, ArrayList<LinkedList<Integer>> adjLists ,int[] distance, int start) {
        ...
        while (!queue.isEmpty()) {
            v = queue.poll();
            LinkedList<Integer> adjList = adjLists.get(v);
